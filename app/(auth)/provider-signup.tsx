...
export default function ProviderSignupScreen() {
  ...
  const renderStep1 = () => (
    <AnimatedEntrance index={0}>
      <View style={{ gap: 24 }}>
        ...
        <Text style={{ fontFamily: 'Cairo_700Bold', fontSize: 18, color: onSurface, marginTop: 8 }}>{t('auth.socialMedia')}</Text>
  ...
  const renderStep2 = () => (
    <AnimatedEntrance index={0}>
      <View style={{ gap: 16 }}>
        <Text style={{ fontFamily: 'Cairo_700Bold', fontSize: 20, color: onSurface, marginBottom: 8 }}>{t('auth.reviewApplication')}</Text>
        ...
        <View style={{ borderRadius: 24, padding: 24, backgroundColor: surfaceContainerLowest, borderWidth: 1, borderColor: outlineVariant }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 16 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontFamily: 'Cairo_700Bold', fontSize: 20, color: onSurface }}>
    ...
    <View style={{ flex: 1, backgroundColor: surfaceBg }}>
      ...
      <View style={{ width: '100%', paddingHorizontal: 24, paddingTop: 56, paddingBottom: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: surfaceBg }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
          ...
          <Text style={{ fontFamily: 'Cairo_700Bold', letterSpacing: -0.5, fontSize: 20, color: onSurface }}>
            ...
            {STEPS.map((label, idx) => (
              <React.Fragment key={label}>
                <View style={{ alignItems: 'center', gap: 8 }}>
                  ...
                  <Text style={{ fontSize: 10, fontFamily: 'Cairo_700Bold', textTransform: 'uppercase', letterSpacing: 0.5, color: idx <= step ? '#862045' : onSurfaceVariant }}>
                    {label}
                  </Text>
                </View>
                {idx < STEPS.length - 1 && (
      ...
      <View style={{
        ...
        {step > 0 && (
          ...
          >
            <Text style={{ fontFamily: 'Cairo_700Bold', color: onSurface }}>{t('auth.back')}</Text>
          </AnimatedButton>
        )}
        <AnimatedButton
          variant="gradient"
