...
export default function ProfileScreen() {
  ...
  const renderSection = (
    ...
    <AnimatedEntrance index={index + 1} delay={150 + index * 50}>
      <Text style={{ fontFamily: 'Epilogue', fontWeight: '700', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1.5, color: colors.onSurfaceVariant, marginBottom: 8, marginStart: 4 }}>{title}</Text>
      <View style={{ backgroundColor: colors.surfaceContainerLowest, borderRadius: Radius.xl, overflow: 'hidden', borderWidth: 1, borderColor: colors.outlineVariant, marginBottom: 20 }}>
        {items.map((item, idx) => (
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontFamily: 'Epilogue', fontWeight: '600', fontSize: 14, color: colors.onSurface }}>{item.title}</Text>
              {item.subtitle ? (
                <Text style={{ color: colors.onSurfaceVariant, fontFamily: 'Manrope', fontSize: 11, marginTop: 2 }}>{item.subtitle}</Text>
              ) : null}
            </View>
        ...
        <View style={{ paddingHorizontal: 16, gap: 16, paddingTop: 8 }}>
            ...
            <View style={{ alignItems: 'center', paddingVertical: 20 }}>
              ...
              <TouchableOpacity onPress={handleEditName} activeOpacity={0.7}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  <Text style={{ fontFamily: 'Epilogue', fontWeight: '800', fontSize: 24, color: colors.onSurface, letterSpacing: -0.5 }}>{displayName}</Text>
              ...
              <View style={{
                ...
                <Text style={{ fontFamily: 'Epilogue', fontWeight: '700', fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, color: colors.primary }}>{t('customer.premiumMember')}</Text>
              </View>
            </View>
          </AnimatedEntrance>
          ...
          <AnimatedEntrance index={1} delay={120}>
            <View style={{ flexDirection: 'row', gap: 12, marginBottom: 8 }}>
              <View style={{
                ...
                <Text style={{ fontFamily: 'Epilogue', fontWeight: '900', fontSize: 28, color: colors.primary, letterSpacing: -1 }}>{stats.totalSaved}</Text>
                <Text style={{ fontFamily: 'Manrope', fontWeight: '700', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1.2, color: colors.onSurfaceVariant, marginTop: 4 }}>{t('customer.totalSaved')}</Text>
              </View>
              <View style={{
                flex: 1,
                backgroundColor: colors.primary,
                ...
                <Text style={{ fontFamily: 'Epilogue', fontWeight: '900', fontSize: 28, color: '#ffffff', letterSpacing: -1 }}>{stats.activeDeals}</Text>
                <Text style={{ fontFamily: 'Manrope', fontWeight: '700', fontSize: 10, textTransform: 'uppercase', letterSpacing: 1.2, color: 'rgba(255,255,255,0.7)', marginTop: 4 }}>{t('customer.activeDeals')}</Text>
              </View>
            </View>
          </AnimatedEntrance>
          ...
          <AnimatedEntrance index={5} delay={400}>
            ...
            >
              ...
              <Text style={{ fontFamily: 'Epilogue', fontWeight: '700', fontSize: 14, color: colors.error }}>{t('customer.logOutAccount')}</Text>
            </TouchableOpacity>
          </AnimatedEntrance>
        </View>
      </ScrollView>
        ...
        <TouchableOpacity activeOpacity={1} onPress={() => setShowNameModal(false)} style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 24 }}>
          <TouchableOpacity activeOpacity={1} onPress={(e) => e.stopPropagation()}>
            <View style={{ backgroundColor: colors.surfaceContainerLowest, borderRadius: Radius.xl, padding: 24, ...Shadows.lg }}>
              <Text style={{ fontFamily: 'Epilogue', fontWeight: '700', fontSize: 18, color: colors.onSurface, marginBottom: 16 }}>{t('customer.personalInfo')}</Text>
              <Text style={{ fontFamily: 'Manrope', fontWeight: '600', fontSize: 12, color: colors.onSurfaceVariant, marginBottom: 8 }}>Display Name</Text>
              <TextInput
                value={editingName}
              ...
              <View style={{ flexDirection: 'row', gap: 12 }}>
                ...
                >
                  <Text style={{ fontFamily: 'Epilogue', fontWeight: '700', fontSize: 14, color: colors.onSurface }}>{t('common.cancel')}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={handleSaveName}
                  style={{ flex: 1, padding: 14, borderRadius: Radius.lg, backgroundColor: colors.primary, alignItems: 'center' }}
                >
                  <Text style={{ fontFamily: 'Epilogue', fontWeight: '700', fontSize: 14, color: colors.onPrimary }}>Save</Text>
                </TouchableOpacity>
              </View>
            </View>
          </TouchableOpacity>
